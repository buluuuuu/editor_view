...
const handleDrop = (e) => {
    ...
    itemList.value.push({...basicItem, ...item, ...{left: e.clientX - rectObj.left - item.width*0.5, top: e.clientY - rectObj.top - item.height*0.5, ref: null, id: Math.round(Math.random()*10000)}})
}
...
<template>
<main class="flex flex-col h-screen w-screen divide-y overflow-hidden">
    <!-- header -->
    <div class="flex flex-row grow-0 h-fit px-9 py-4 justify-between align-middle text-center">
        <div class="flex flex-row justify-between flex-1">
            ...
            <div class="flex flex-row gap-[18px] justify-between">
                ...
                <Button @click="handleUndo">
                    <svg  class="icon w-6 h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M15.13 19.0601H7.13C6.72 19.0601 6.38 18.7201 6.38 18.3101C6.38 17.9001 6.72 17.5601 7.13 17.5601H15.13C17.47 17.5601 19.38 15.6501 19.38 13.3101C19.38 10.9701 17.47 9.06006 15.13 9.06006H4.13C3.72 9.06006 3.38 8.72006 3.38 8.31006C3.38 7.90006 3.72 7.56006 4.13 7.56006H15.13C18.3 7.56006 20.88 10.1401 20.88 13.3101C20.88 16.4801 18.3 19.0601 15.13 19.0601Z"/>
                        <path d="M6.43 11.5599C6.24 11.5599 6.05 11.4899 5.9 11.3399L3.34 8.77988C3.05 8.48988 3.05 8.00988 3.34 7.71988L5.9 5.15988C6.19 4.86988 6.67 4.86988 6.96 5.15988C7.25 5.44988 7.25 5.92988 6.96 6.21988L4.93 8.24988L6.96 10.2799C7.25 10.5699 7.25 11.0499 6.96 11.3399C6.82 11.4899 6.62 11.5599 6.43 11.5599Z"/>
                    </svg>
                </Button>
                <!-- 重做 -->
                <Button @click="handleRedo">
                    <svg class="icon w-6 h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M16.87 19.0601H8.87C5.7 19.0601 3.12 16.4801 3.12 13.3101C3.12 10.1401 5.7 7.56006 8.87 7.56006H19.87C20.28 7.56006 20.62 7.90006 20.62 8.31006C20.62 8.72006 20.28 9.06006 19.87 9.06006H8.87C6.53 9.06006 4.62 10.9701 4.62 13.3101C4.62 15.6501 6.53 17.5601 8.87 17.5601H16.87C17.28 17.5601 17.62 17.9001 17.62 18.3101C17.62 18.7201 17.29 19.0601 16.87 19.0601Z"/>
        ...
        <div class="flex flex-row flex-1 justify-center align-center">
            <InputText class="text-[#5f5f5f] text-center" type="text" v-model="projectInfo.name"/>
        </div>
        <div class="flex flex-row flex-1 justify-between">
            <!-- 保存 -->
            <div class="flex flex-row justify-between">
                <div class="flex flex-row items-center mr-[1rem]" :style="{visibility: saveSuccess}">
                    <svg class="h-[1.128rem] mr-[0.35rem]" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M7.75002 19H3.25C2.00735 19 0.999992 17.9926 1 16.75L1.00009 3.24999C1.00009 2.00736 2.00745 1 3.25009 1H13.3754C14.618 1 15.6254 2.00736 15.6254 3.25001V9.43752M11.1254 15.8125L13.1879 17.875L18.4379 12.25" stroke="#5F5F5F" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                    <span class="inline-block text-xs text-[#5F5F5F]">保存成功</span>
                </div>
                <Button @click="handleSave" class="py-2 px-[1.6rem] border border-[#eeaa1e] hover:bg-[#eeaa1e] rounded-md text-sm text-[#eeaa1e] hover:text-white font-normal">保存设计稿</Button>
            </div>
            <!-- 生成 -->
            <div class="flex flex-row">
                <div class="flex flex-row mr-[0.7rem] gap-[4px] justify-between items-center">
                    <Button class="w-8 h-8 rounded-md" @click="()=>{imgNum--}">
                        <svg width="32" height="34" viewBox="0 0 32 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <rect width="32" height="34" rx="5" fill="#F6F4F0"/>
                            <path d="M18.8047 22.416C19.0651 22.1493 19.0651 21.7168 18.8047 21.4501L14.6094 17.1518L18.8047 12.8535C19.0651 12.5867 19.0651 12.1543 18.8047 11.8875C18.5444 11.6208 18.1223 11.6208 17.862 11.8875L13.1953 16.6688C12.9349 16.9355 12.9349 17.368 13.1953 17.6347L17.862 22.416C18.1223 22.6828 18.5444 22.6828 18.8047 22.416Z" fill="#8B8B8B"/>
                        </svg>
                    </Button>
                    <span class="inline-block px-[1.6rem] h-[2.25rem] leading-[2.25rem] bg-[#eeaa1e] rounded-md text-sm text-[#ffffff] font-normal">{{imgNum}}张</span>
                    <Button class="w-8 h-8 bg-[#eeaa1e] rounded-md text-sm text-[#ffffff] font-normal" @click="()=>{imgNum++}">
                        <svg width="32" height="34" viewBox="0 0 32 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <rect x="32" y="34" width="32" height="34" rx="5" transform="rotate(-180 32 34)" fill="#F6F4F0"/>
                            <path d="M13.1953 11.584C12.9349 11.8507 12.9349 12.2832 13.1953 12.5499L17.3906 16.8482L13.1953 21.1465C12.9349 21.4133 12.9349 21.8457 13.1953 22.1125C13.4556 22.3792 13.8777 22.3792 14.138 22.1125L18.8047 17.3312C19.0651 17.0645 19.0651 16.632 18.8047 16.3653L14.138 11.584C13.8777 11.3172 13.4556 11.3172 13.1953 11.584Z" fill="#8B8B8B"/>
                        </svg>
                    </Button>
                </div>
                <Button
    ...
    <div class="flex flex-row h-0 items-stretch divide-x grow">
        ...
        <div class="relative flex flex-col w-[20vw] px-4 pt-7 z-[100]">
            ...
            <material-container
             ...
             @selectStyle="(name)=>{projectInfo.style.add(name)}"
             @selectType="(name)=>{projectInfo.type.add(name)}"
             @deleteStyle="(name)=>{projectInfo.style.delete(name)}"
             @deleteType="(name)=>{projectInfo.type.delete(name)}"
             :class="{hidden: leftBtn=='upload'}">
            </material-container>
        </div>
        <!-- 中间画布 -->
        <div @wheel="(e)=>handleZoom(e)" class="relative flex flex-col w-[64vw] bg-[#f6f4f0] items-center z-0">
            <!-- 素材编辑栏 -->
            <div :style="{visibility: selected}" class="relative flex flex-row w-full h-fit px-5 py-4 gap-4 bg-white items-center paramsInput z-[100]">
                <!-- 正向影响力 -->
                <div class="flex flex-row items-center">
                    <label for="positive" class="text-xs text-[#313131]">正向影响力：</label>
                    <InputNumber 
                     inputId="positive" 
                     :minFractionDigits="1" 
                     :maxFractionDigits="1"
                     ...
                     <div class="flex flex-col">
                        <svg @click="()=>w_pos+=0.1" class="w-[0.7rem] cursor-pointer arrow" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M2.54049 7.08451C2.3615 6.90552 2.3615 6.61532 2.54049 6.43633L5.06133 3.91549C5.24032 3.7365 5.53052 3.7365 5.70951 3.91549L8.23034 6.43633C8.40933 6.61532 8.40933 6.90552 8.23034 7.08451C8.05135 7.2635 7.76115 7.2635 7.58216 7.08451L5.38542 4.88776L3.18867 7.08451C3.00968 7.2635 2.71948 7.2635 2.54049 7.08451Z"/>
                        </svg>
                        <svg @click="()=>w_pos-=0.1" class="w-[0.7rem] cursor-pointer arrow" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M8.23035 4.25924C8.40934 4.43823 8.40934 4.72843 8.23035 4.90742L5.70952 7.42826C5.53053 7.60725 5.24033 7.60725 5.06134 7.42826L2.5405 4.90742C2.36151 4.72843 2.36151 4.43823 2.5405 4.25924C2.71949 4.08025 3.00969 4.08025 3.18868 4.25924L5.38543 6.45599L7.58217 4.25924C7.76116 4.08025 8.05136 4.08025 8.23035 4.25924Z"/>
                        </svg>
                     </div>
                ...
                <!-- 负向影响力 -->
                <div class="flex flex-row items-center">
                    <label for="negative" class="text-xs text-[#313131]">负向影响力：</label>
                    <InputNumber 
                     inputId="negative" 
                     :minFractionDigits="1" 
                     :maxFractionDigits="1"
                     ...
                     :max="2.9" />
                     <div class="flex flex-col">
                        <svg @click="()=>w_neg+=0.1" class="w-[0.7rem] cursor-pointer arrow" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M2.54049 7.08451C2.3615 6.90552 2.3615 6.61532 2.54049 6.43633L5.06133 3.91549C5.24032 3.7365 5.53052 3.7365 5.70951 3.91549L8.23034 6.43633C8.40933 6.61532 8.40933 6.90552 8.23034 7.08451C8.05135 7.2635 7.76115 7.2635 7.58216 7.08451L5.38542 4.88776L3.18867 7.08451C3.00968 7.2635 2.71948 7.2635 2.54049 7.08451Z"/>
                        </svg>
                        <svg @click="()=>w_neg-=0.1" class="w-[0.7rem] cursor-pointer arrow" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M8.23035 4.25924C8.40934 4.43823 8.40934 4.72843 8.23035 4.90742L5.70952 7.42826C5.53053 7.60725 5.24033 7.60725 5.06134 7.42826L2.5405 4.90742C2.36151 4.72843 2.36151 4.43823 2.5405 4.25924C2.71949 4.08025 3.00969 4.08025 3.18868 4.25924L5.38543 6.45599L7.58217 4.25924C7.76116 4.08025 8.05136 4.08025 8.23035 4.25924Z"/>
                ...
                <div class="flex flex-row items-center mr-2">
                    <label for="influence" class="text-xs text-[#313131]">影响因子：</label>
                    <InputNumber 
                     inputId="influence" 
                     :minFractionDigits="1" 
                     :maxFractionDigits="1"
                     ...
                     :max="1" />
                     <div class="flex flex-col">
                        <svg @click="()=>t_i+=0.1" class="w-[0.7rem] cursor-pointer arrow" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M2.54049 7.08451C2.3615 6.90552 2.3615 6.61532 2.54049 6.43633L5.06133 3.91549C5.24032 3.7365 5.53052 3.7365 5.70951 3.91549L8.23034 6.43633C8.40933 6.61532 8.40933 6.90552 8.23034 7.08451C8.05135 7.2635 7.76115 7.2635 7.58216 7.08451L5.38542 4.88776L3.18867 7.08451C3.00968 7.2635 2.71948 7.2635 2.54049 7.08451Z"/>
                        </svg>
                        <svg @click="()=>t_i-=0.1" class="w-[0.7rem] cursor-pointer arrow" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M8.23035 4.25924C8.40934 4.43823 8.40934 4.72843 8.23035 4.90742L5.70952 7.42826C5.53053 7.60725 5.24033 7.60725 5.06134 7.42826L2.5405 4.90742C2.36151 4.72843 2.36151 4.43823 2.5405 4.25924C2.71949 4.08025 3.00969 4.08025 3.18868 4.25924L5.38543 6.45599L7.58217 4.25924C7.76116 4.08025 8.05136 4.08025 8.23035 4.25924Z"/>
                ...
                <div>
                    <svg class="hint" width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M10.5 8.85C10.928 8.85 11.275 9.19698 11.275 9.625V14.875C11.275 15.303 10.928 15.65 10.5 15.65C10.072 15.65 9.725 15.303 9.725 14.875V9.625C9.725 9.19698 10.072 8.85 10.5 8.85ZM11.275 6.125C11.275 6.55302 10.928 6.9 10.5 6.9C10.072 6.9 9.725 6.55302 9.725 6.125C9.725 5.69698 10.072 5.35 10.5 5.35C10.928 5.35 11.275 5.69698 11.275 6.125ZM19.15 10.5C19.15 15.2773 15.2773 19.15 10.5 19.15C5.72274 19.15 1.85 15.2773 1.85 10.5C1.85 5.72274 5.72274 1.85 10.5 1.85C15.2773 1.85 19.15 5.72274 19.15 10.5ZM10.5 17.6C14.4212 17.6 17.6 14.4212 17.6 10.5C17.6 6.57878 14.4212 3.4 10.5 3.4C6.57878 3.4 3.4 6.57878 3.4 10.5C3.4 14.4212 6.57878 17.6 10.5 17.6Z" fill="#5F5F5F" stroke="white" stroke-width="0.2"/>
                    </svg>
                    <div class="absolute hidden flex-auto mt-4 translate-x-[calc(-50%+10.5px)]">
                        <div class="relative w-5 h-5 rotate-[-45deg] left-1/2 translate-x-[-50%] translate-y-[50%] bg-white shadow-[4px_4px_4px_0_rgba(0,0,0,0.25)] z-0"></div>
                        <div class="relative w-[28.25rem] h-[12.6rem] bg-white py-5 px-7 text-[0.625rem] rounded-lg shadow-[4px_4px_4px_0_rgba(0,0,0,0.25)] z-1 text-[--c-black]">
                            <p class="font-semibold">
                                <span class="text-[#BE8208]">正向影响力（w_pos）：</span>增加该素材在生成过程中对于其所处区域的影响程度
                            </p>
                            <p>范围：(0,1) ，默认0.3，建议不进行调整。若想获得更多输出，可尝试微调。</p>
                            <br/>
                            <p class="font-semibold">
                                <span class="text-[#BE8208]">负向影响力（w_neg）：</span>降低该素材在生成过程中对于其所处区域以外的区域的影响程度
                            </p>
                            <p>范围：(0,3) ，默认1.0，建议不进行调整，若想获得更多输出，可尝试微调。</p>
                            <br/>
                            <p class="font-semibold">
                                <span class="text-[#BE8208]">变化因子（t_i）：</span>表明该素材的保真度与和谐度
                            </p>
                            <p>
                ...
                <div
                 ...
                 class="flex flex-col min-w-full min-h-full items-center shrink-0 justify-center p-8">
                    <div
                     ...
                     :style="{width: `${projectInfo.width}px`, height: `${projectInfo.height}px`, transform: `scale(${canvas_scale})`}"
                     class="relative bg-white canvas overflow-hidden shrink-0">
                        ...
                        <div
                         ...
                         :key="item"
                         :style="{position: `absolute`, zIndex: `${index}`, transform: `translate(${item.left}px, ${item.top}px)`}"
                        ...
                        >
                            <Drager
                             ...
                             @resize-end="(e)=>handleResizeEnd(e,item)"
                             @rotate-end="(e)=>handleRotateEnd(e,item)"
        ...
        <!-- 右边图层 -->
        <div class="relative flex flex-col w-[12vw] px-2 py-[2px] divide-y z-[100]">
            <div class="w-full py-2.5 px-3.5 font-medium text-base">图层</div>
            <!-- 把vh改成rem比较好？待修 -->
            <draggable
             :list="itemList"
             chosen-class="bg-gray-100">
                <template #item="{element}">
                    <div class="h-[4vh] m-2.5 rounded-md">
                        <drag-item item-key="element" :img_name="element.name" :img="{img_url: element.img_url, id: element.img_id}"></drag-item>
                    </div>
                </template>
            </draggable>
        </div>
